...
bool spcm_bCreateThread (SPCM_THREADFUNCTION* pfnThread, SPCM_THREAD_HANDLE* phThread, void* pvArguments)
    ...
    (*phThread) = CreateThread (nullptr, 0, pfnThread, pvArguments, 0, &dwThreadId);

    return ((*phThread) != nullptr);
    }
...
void* pvAllocMemPageAligned (uint64 qwBytes)
    ...
    if (qwBytes >= 4096)
        return VirtualAlloc (nullptr, static_cast<size_t>(qwBytes), MEM_RESERVE | MEM_COMMIT, PAGE_READWRITE);
    else
        {
        void* pvMem = _aligned_malloc (static_cast<size_t>(qwBytes), 4096);
        if (pvMem == nullptr)
            return nullptr;
        memset (pvMem, 0, static_cast<size_t>(qwBytes));
        return pvMem;
        }
    }
